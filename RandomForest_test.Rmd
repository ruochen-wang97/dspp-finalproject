...
claims_long <- claims_long %>%
  select(initial_type, initial_num, continued_type.x, continued_num, 
         date, sah_date, days_since_peak_deaths, sum_conf, sum_deaths, 
         Incident_Rate, Mortality_Rate, region, topind_gdp, topindemp_1, 
         topindemp_2, topindemp_3, totpop_2018, pctpop_work, pctpop_hs, 
...
recipe_claims <- recipe(formula = initial_num ~ continued_num + 
                          ...
                          sum_deaths + Incident_Rate + Mortality_Rate + 
  ...
  if(model == "rf") {
    model_claims <- rand_forest(mode = "regression",
      ...
      fit(formula = initial_num ~ continued_num + days_since_sah + 
            days_since_peak + sum_conf + sum_deaths + Incident_Rate + 
            Mortality_Rate + region + topind_gdp + topindemp_1 + 
            topindemp_2 + topindemp_3 + totpop_2018 + pctpop_work + 
            pctpop_hs + personal_inc2019 + gov_republican,
