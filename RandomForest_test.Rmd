...
initial_long <- claims_data %>%
  pivot_longer(cols = initial_claims_2020_01_04:initial_claims_2020_04_18, names_to = "initial_type", values_to = "initial_num") %>%
  left_join(date_var)

continued_long <- claims_data %>%
  pivot_longer(cols = continued_claims_2020_01_04:continued_claims_2020_04_18, names_to = "continued_type", values_to = "continued_num") %>%
  left_join(date_var) %>%
  select(state, date, continued_type, continued_num)

claims_long <- left_join(initial_long, continued_long, by = c("state","date"))
anti_join(initial_long, continued_long)

# select variables
claims_long <- claims_long %>%
  select(initial_type, initial_num, continued_type.x, continued_num, date, sah_date,
         days_since_peak_deaths, sum_conf, sum_deaths, Incident_Rate, 
  ...
  mutate(days_since_sah = ifelse(date < as.Date("2020-04-18"), as.Date("2020-04-11") - ymd(sah_date), as.Date("2020-04-18") - ymd(sah_date))) %>%
  mutate(days_since_sah = replace_na(days_since_sah, 0)) %>%
  mutate(days_since_peak = ifelse(date < as.Date("2020-04-18"), days_since_peak_deaths - 21, days_since_peak_deaths - 14)) %>%
...
recipe_claims <- recipe(formula = initial_num ~ continued_num +
                          days_since_sah + days_since_peak + sum_conf + 
...
train_claims <- function(model, split, try, tree) {
  ...
  if(model == "rf") {
    model_claims <- rand_forest(mode = "regression",
      ...
      set_engine("ranger",
                 importance = "impurity") %>%
      fit(formula = initial_num ~ continued_num +
                          days_since_sah + days_since_peak + sum_conf + 
                          sum_deaths + Incident_Rate + Mortality_Rate + region + 
                          topind_gdp + topindemp_1 + topindemp_2 + topindemp_3 + 
                          totpop_2018 + pctpop_work + pctpop_hs + gov_republican,
