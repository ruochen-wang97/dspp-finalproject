---
title: "Unemployment Claims Data Construction"
...
```{r multiple_sources_compilation, echo=T, message=F, warning=F}
...
```{r predicted_peak_deaths,  echo=T, message=F, warning=F}
...
```{r Ed_attainment_data_merge,  echo=T, message=F, warning=F}
...
```{r working_pop_data,  echo=T, message=F, warning=F}
...
```{r Dem/Rep_governor,  echo=T, message=F, warning=F}
...
```{r predicted_deaths_IHME_data,  echo=T, message=F, warning=F}
# load & add governor party data
ihme_data <- read_excel("data/raw/Covid_data_April10.xlsx")
ihme_data$date <- as.character(ihme_data$date)

ihme_data <- ihme_data %>%
              rename(state = "location_name") %>%
              select(state, date, deaths_mean, totdea_mean) %>%
              filter(date == "2020-04-10")

clean_data <-
  left_join(clean_data, ihme_data, by = "state")
anti_join(clean_data, ihme_data)
```

```{r actual_deaths_till_Apr10, echo=T, message=F, warning=F}

# Loading packages
library(httr)
library(RCurl)

# Reading the JHU Covid data from Github for April 10, 2020.
jhufile_1 = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/04-10-2020.csv"
...
clean_data <-
  left_join(clean_data, jhu_data, by = "state")
anti_join(clean_data, jhu_data)
```


```{r incidence_mortality_rates_Apr12, echo=T, message=F, warning=F}
# April 12 data on rates of incidence and mortality
jhufile_2 = "https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports_us/04-12-2020.csv"

rates_data <- read.csv(url(jhufile_2))

jhu_data_1 <- rates_data %>%
  rename(state = "Province_State") %>%
  select(FIPS, state, Deaths, Lat, Long_, Incident_Rate, Mortality_Rate) %>%
  filter(FIPS < 60)
