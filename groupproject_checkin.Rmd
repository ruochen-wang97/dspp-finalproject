...
ggplot(data = clean_data, mapping = aes(x = peak, y = as.factor(region))) +
  ...
  labs(title = 
         paste("Distribution of Days Since Coronavirus Peak Deaths by Region"),
       caption = "Source:???",
       x = "Days since peak deaths",
       y = "Region") +
  theme_minimal() 
...
```{r}
clean_data %>%
  mutate(perc_claim = (`continued_claims_2020-04-18`/totpop_2018) * 100) %>%
  ggplot() +
  geom_point(mapping = aes(x = perc_claim, y = Incident_Rate, color = as.character(gov_republican)))
