...
# find top industry wrt employment
occ <- occ %>%
  mutate(topind_emp = colnames(occ)[apply(occ, 1, which.max)]) %>%
  select(state, topind_emp)
```
...
### Stay at home dates and predicted peaks by IHME model.

```{r sah_clean, echo=T, message=F, warning=F}
...
clean_data <-
  left_join(clean_data, pop, by = "state")
anti_join(clean_data, pop)
