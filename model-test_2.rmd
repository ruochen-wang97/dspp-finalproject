...
# select variables - testing outcome of initial claims filed within working population by state (I seem to have coded the outcome var wrong and need help!)
claims_long <- claims_long %>%
  select(initial_type, initial_num, continued_type.x, continued_num, 
         ...
         Mortality_Rate, region, sah_yn, peak_death_yn, topind_gdp, topindemp_1, topindemp_2, 
         topindemp_3, totpop_2018, pctpop_work, pctpop_hs, totpop_work,
  ...
  mutate(days_since_peak =
    ...
    mutate(pct_claims_filed = (initial_num/totpop_work)*100)%>%
  mutate(gov_republican = factor(gov_republican)) %>%
  select(-sah_date, -days_since_peak_deaths, -totpop_work)
```
...
recipe_claims <- recipe(formula = pct_claims_filed ~ continued_num + 
...
train_claims <- function(model, split, try, tree) {
  ...
  if(model == "rf") {
    model_claims <- rand_forest(mode = "regression",
      ...
      set_engine("ranger",
                 importance = "impurity") %>%
      fit(formula = pct_claims_filed ~ continued_num + days_since_sah + 
  ...
  rmse <- bind_cols(
    ...
    rmse(truth = pct_claims_filed, estimate = .pred) %>%
    pull(.estimate)
...
bind_cols(
  ...
  metrics(truth = pct_claims_filed, 
          estimate = .pred)
```
