...
income <- income %>%
  ...
  rename(income = "2019", state = GeoName) %>%
  select(-GeoFips)
```
...
pop <- pop %>%
  ...
  rename(fips = STATE, state = NAME, pop_2019 = POPESTIMATE2019, pop_over18 = POPEST18PLUS2019, pcnt_pop_over18 = PCNT_POPEST18PLUS)
```
...
occ <- occ %>%
  ...
  rename(fips = area, state = area_title, mean_hourly_wage = h_mean, med_hourly_wage = h_median )
```
...
regions <- regions %>%
  select(State, Region) %>%
  rename(state = State, region_name = Region)
```
